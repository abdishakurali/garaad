...
const faqs: FAQ[] = [
    ...
    },
    {
        ...
        answer: "Haa, barnaamijkan waxaa loogu talagalay dadka mashquulka ah. Mid kasta oo ka mid ah 3-da bilood waxay u baahan tahay qiyaastii 20-35 saacadood oo shaqo xooggan ah. Waxaad u qaybin kartaa habeenkii iyo maalmaha fasaxa. Furaha guushu waa joogteyn—xitaa 2 saacadood maalintii way ku gaarsiin karaan hadafkaba.",
    },
    {
...
export function FAQSection({ className }: { className?: string }) {
    ...
    const { data: faqApi } = useSWR<FaqApiResponse>(
        `${API_BASE_URL}/api/public/faqs/?placement=landing`,
        fetcher,
    ...
    const items: FAQ[] =
        faqApi?.faqs && faqApi.faqs.length > 0
            ? faqApi.faqs.map((f) => ({ id: f.id, question: f.question, answer: f.answer }))
            : faqs;
