...
const nextConfig = {
  ...
  async headers() {
    const cspHeader = `
      default-src 'self';
      script-src 'self' 'unsafe-eval' 'unsafe-inline' https://js.stripe.com https://*.posthog.com *.vercel-scripts.com https://player.vimeo.com;
      ...
      connect-src 'self' https://api.garaad.org wss://api.garaad.org https://*.posthog.com https://api.stripe.com https://js.stripe.com https://www.transparenttextures.com http://localhost:8000 http://127.0.0.1:8000;
    `.replace(/\s{2,}/g, ' ').trim();
